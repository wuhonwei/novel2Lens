...
def test_build_shot_references_one_portrait_per_character():
    ...
    slots = [
        ...
        {"index": 2, "kind": "character", "asset_id": "c1", "image_key": "half", "position": "中"},
    ]
    ...
    char_refs = [r for r in refs if r["kind"] == "character" or r["image_key"] in ("half", "full")]
    assert len(char_refs) == 1
    assert char_refs[0]["image_key"] == "half"
    assert char_refs[0]["image_role"] == "人物半身图"
    assert char_refs[0]["uploaded"] is True
    assert refs[0]["status_zh"] == "尚未上传"


def test_build_shot_references_includes_prop_and_full_only():
