...
async def fake_chat_json(messages, **_kwargs):
    ...
    if "完整性审计" in text or "第二遍" in text or "查漏补缺" in text:
        # first audit incomplete, second complete — track via registry richness in prompt
        if "眉眼清俊" in text or "半块玉佩" in text:
            return PASS3, False
        return PASS2, False
    if "登记总表" in text or "第一遍" in text or "人物/场景登记员" in text or "通读材料" in text:
        return PASS1, False
    return {"proposals": []}, False


def test_upload_runs_two_pass_registry_scan(tmp_path, monkeypatch):
    ...
    with TestClient(app) as client:
        ...
        assert "半块玉佩" in prop["desc_zh"]
        # props must not pretend to be characters in API
        assert prop["half_path"] == ""
        assert prop["full_path"] == ""
        assert body["project"].get("registry_scan") or body.get("result", {}).get("passes")
        scan = body.get("result") or body["project"].get("registry_scan")
        if isinstance(scan, dict) and "passes" in scan:
            assert len(scan["passes"]) >= 2
